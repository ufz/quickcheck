...
could not be practically generated by the framework).

Instead, one should also provide carefully chosen test cases that put the
tested code under stress.

This can be done by adding input data for these test cases to the
quickcheck::Property::_fixedInputs vector. Each input data present in this
vector will be used as input arguments for your property before any
randomly-generated input arguments each time quickcheck::Property::check is
called.

The element type of this vector is the 5-tuple quickcheck::Property::Input
